...
def _rejected_rows(
    ...
    *,
    ...
    for index, row in enumerate(rows):
        ...
        # 표준화 Rejected는 errors 목록을 사용하고, 정규화 Rejected는
        # rejection_reason 문자열을 사용한다. 두 결과를 같은 MongoDB 필드
        # 형식으로 저장하기 위해 문자열 사유도 목록으로 바꾼다.
        if not rejected_reasons:
            ...
            elif isinstance(reason_text, list):
                rejected_reasons = [str(reason) for reason in reason_text]
...
def write_rejected_rows_to_mongodb(
    ...
    *,
    ...
    standardization_run_id = str(
        rejected_standardization_result.get("run_id", "")
    ).strip()
    normalization_run_id = str(
        rejected_normalization_result.get("run_id", "")
    ).strip()
    if standardization_run_id != normalization_run_id:
        raise RejectedMongoDataError(
            "표준화 결과와 정규화 결과의 run_id가 다릅니다."
        )
    ...
    try:
        ...
        standardization_deleted_count = standardization_collection.delete_many(
            {"run_id": standardization_run_id}
        ).deleted_count
        normalization_deleted_count = normalization_collection.delete_many(
        ...
        return {
            ...
            "standardization_deleted_count": standardization_deleted_count,
            "normalization_deleted_count": normalization_deleted_count,
        }
    finally:
        client.close()
