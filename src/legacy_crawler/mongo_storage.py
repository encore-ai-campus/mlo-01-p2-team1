"""MongoDB staging history plus guarded page-append production storage."""
...
class MongoStorage:
    ...
    @property
    def production(self) -> Collection[Mapping[str, Any]]:
        return self.database[PRODUCTION_COLLECTION]

    def _rename_collection(self, source: str, target: str) -> None:
        if target in self.database.list_collection_names():
            raise ProductionProtectionError(f"rename target already exists: {target}")
        result = self._client.admin.command(
            {
                "renameCollection": f"{self.settings.mongodb_database}.{source}",
                "to": f"{self.settings.mongodb_database}.{target}",
            }
        )
        if result.get("ok") != 1:
            raise MongoStorageError(f"collection rename failed: {source} -> {target}")

    def initialize_page_append_production(self, *, timestamp: str) -> str:
        """Preserve legacy production and create an empty append target once."""

        names = self.database.list_collection_names()
        if PRODUCTION_COLLECTION not in names:
            raise ProductionProtectionError("legacy_records does not exist")
        backup = f"legacy_records_backup_pre_page_append_{safe_run_id(timestamp)}"
        self._rename_collection(PRODUCTION_COLLECTION, backup)
        try:
            self.database.create_collection(PRODUCTION_COLLECTION)
            self.production.create_index(
                [("record_id", ASCENDING)], name="uq_record_id", unique=True
            )
        except Exception:
            if PRODUCTION_COLLECTION in self.database.list_collection_names():
                failed = f"legacy_records_failed_initialization_{safe_run_id(timestamp)}"
                self._rename_collection(PRODUCTION_COLLECTION, failed)
            self._rename_collection(backup, PRODUCTION_COLLECTION)
            raise
        return backup

    def rollback_page_append_initialization(
        self, *, backup_collection: str, timestamp: str
    ) -> str:
        """Preserve the failed append target and restore the untouched backup."""

        failed = f"legacy_records_failed_page_append_{safe_run_id(timestamp)}"
        self._rename_collection(PRODUCTION_COLLECTION, failed)
        self._rename_collection(backup_collection, PRODUCTION_COLLECTION)
        return failed
    ...
    def append_page(
        ...
        *,
        ...
        """Append one cursor page to production without weakening other guards."""

        indexes = {item["name"]: item for item in self.production.list_indexes()}
        unique = indexes.get("uq_record_id")
        if not unique or unique.get("key") != {"record_id": 1} or not unique.get("unique"):
            raise ProductionProtectionError("production uq_record_id index is missing")
        documents = [
        ...
        if len(record_ids) != len(set(record_ids)):
            raise SourceContractError("page contains duplicate record_id")
        source_rows = [document["source_row_no"] for document in documents]
        if len(source_rows) != len(set(source_rows)):
            raise SourceContractError("page contains duplicate source_row_no")
        if record_ids:
            existing = list(
                self.production.find(
                    {"record_id": {"$in": record_ids}},
                    {"record_id": 1, "_ingest.run_id": 1},
                )
            )
            if existing:
                raise SourceContractError(
                    "page overlaps existing production record_id values"
                )
            result = self.production.insert_many(documents, ordered=True)
            return len(result.inserted_ids)
        return 0

    def rollback_appended_page(self, run_id: str) -> int:
        """Remove only an uncommitted page run after its processing failed."""

        result = self.production.delete_many({"_ingest.run_id": run_id})
    ...
    def create_run(
        ...
        *,
        ...
        document = {
            ...
            "staging_collection": None if append_mode else self.staging_name(run_id),
    ...
    def mark_ready_after_page_append(
        self,
        run_id: str,
        *,
        ready_at: str,
        ...
        if not validation.passed or validation.run_id != run_id:
            raise ProductionProtectionError("page READY requires passing validation")
        run = self.runs.find_one({"run_id": run_id})
        if not run or run.get("state") != RunState.VALIDATING.value:
            raise MongoStorageError("page run is not validating")
        expected_rows = run.get("expected_rows")
        actual_rows = self.production.count_documents({"_ingest.run_id": run_id})
        if actual_rows != expected_rows:
            raise ProductionProtectionError("page READY row count mismatch")
        result = self.runs.update_one(
            {"run_id": run_id, "state": RunState.VALIDATING.value},
            {"$set": {"state": RunState.READY.value, "ready_at": ready_at}},
        )
        if result.matched_count != 1:
            raise MongoStorageError("page validating -> ready transition rejected")

    def mark_page_runs_rolled_back(self, run_ids: Sequence[str], *, error: str) -> None:
        if not run_ids:
            return
        self.runs.update_many(
            {
                "run_id": {"$in": list(run_ids)},
                "state": {"$in": [RunState.LOADING.value, RunState.VALIDATING.value, RunState.READY.value]},
            },
            {
                "$set": {
                    "state": RunState.FAILED.value,
