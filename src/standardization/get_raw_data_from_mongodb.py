...
def _environment_from_dotenv(dotenv_path: str | Path | None) -> Mapping[str, str]:
    ...
    if not path.is_file():
        raise RawMongoConfigurationError(f".env 파일이 없습니다: {path}")
    try:
            ...
            for manifest in manifests:
                ...
                if not isinstance(run_id, str) or not run_id.strip():
                    raise RawMongoDataError(
                        "pending manifest에 유효한 run_id가 없습니다."
                    )
                pending_run_ids.append(run_id)

            existing_run_ids = set(
                raw_data_collection.distinct(
                    "_ingest.run_id",
