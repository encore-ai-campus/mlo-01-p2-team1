...
수집된 데이터는 Medallion 아키텍처를 따라 처리됩니다.

1. 원본 레거시 데이터와 메타데이터는 최대한 원본 그대로 Bronze Layer에 저장합니다.
2. 컬럼명, 데이터 타입, 코드 및 식별자 체계를 표준화하고, 고도화된 품질 검증 파이프라인을 통해 정상적으로 정제된 데이터와 추가 검증·교정이 필요한 격리(Rejected) 데이터를 명확히 분류하여 Silver Layer에 저장합니다.
3. 검증을 마친 고품질 정제 데이터의 패턴을 분석하여, 격리된 데이터의 결측치(Null) 및 이상치(Error)에 대해 최적의 보정값을 자동으로 추론·추천할 수 있는 AI-Ready Data를 구축하여 Gold Layer에 저장합니다.

본 프로젝트의 목표는 기업이 디지털 전환(DX) 과정에서 확신을 갖고 활용할 수 있는 자율 교정형 데이터 관리 생태계를 조성하는 것입니다.
다만 이번 구현에서는 레거시 데이터의 표준화·정규화 파이프라인 확립과 결측/오류 데이터에 대한 지능형 보정값 추천 기반을 견고히 다지는 데 초점을 맞춥니다.
...
| 신성민 | API 기반 데이터 수집부터 MongoDB 적재·증분 자동화·검증까지 직접 구현하며, 단순 수집이 아닌 안정적인 Bronze 파이프라인 운영 구조를 이해할 수 있었다. |
| 오재석 | 중간중간에 수정을 하느라 작업이 예정된 것보다 계속 딜레이가 되어 완성도가 아쉽습니다. 초반 설계가 탄탄해야 하는것을 다시 느꼈습니다 |
| 이인건 | 대규모 데이터 처리에서 각 단계별 체크포인트를 두고 DB 적재 시 트랜잭션을 유지하는 것이 효율적이라는 것을 알게되었고, 정제과정에서 raw데이터의 출처는 일관되어야 함을 느꼈다. |
