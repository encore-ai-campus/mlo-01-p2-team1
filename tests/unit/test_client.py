...
def response(payload: object, *, status: int = 200) -> HttpBody:
    if (
        ...
        and {"items", "count", "has_more"}.issubset(payload)
    ):
        payload = {
...
class BronzeRelayClientTests(unittest.TestCase):
    ...
    def test_initial_cursor_is_sent_without_offset_or_page_parameters(self) -> None:
        payload = {
            "items": [],
            "count": 0,
            "has_more": False,
        ...
        list(
            client.iter_record_pages(
                dataset_id="dataset",
                api_key="secret",
                initial_cursor="stored cursor",
            )
        )

        url = transport.requests[0].full_url
        self.assertIn("cursor=stored+cursor", url)
        self.assertIn("limit=1000", url)
        self.assertNotIn("offset=", url)
        self.assertNotIn("page=", url)
        self.assertNotIn("release_slot=", url)
    ...
    def test_snapshot_change_is_rejected(self) -> None:
        ...
        changed = {
            ...
            "count": 1,
            "has_more": False,
            "next_cursor": "checkpoint-2",
            "checkpoint": "checkpoint-2",
            "released_rows": 2,
        }
        client = BronzeRelayClient(
            settings(), transport=QueueTransport([response(first), response(changed)])
        )
        iterator = client.iter_record_pages(dataset_id="dataset", api_key="secret")
        next(iterator)
        with self.assertRaises(IncompleteCollectionError):
    ...
    def test_rejected_resumed_checkpoint_does_not_fallback_to_page_one(self) -> None:
        payload = {
            "items": [],
            "count": 0,
            "has_more": False,
        ...
        with self.assertRaises(IncompleteCollectionError):
            next(
                client.iter_record_pages(
                    ...
                    api_key="secret",
                    initial_cursor="stored-cursor",
                    expected_checkpoint="different-checkpoint",
                )
            )
        self.assertEqual(len(transport.requests), 1)
        self.assertIn("cursor=stored-cursor", transport.requests[0].full_url)
